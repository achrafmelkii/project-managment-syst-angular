...
//         isActive: this.user.isActive,
//         skills:
//           this.user.skills?.map((skill: Skill) => skill.name).join(', ') || '',
//       });
//       this.imagePreview =
//         this.user.image ||
//         'https://www.iconpacks.net/icons/2/free-user-icon-3296-thumb.png';
...
//       this.errorMessage = null;
//       this.successMessage = null;
//       // Optionally reset image preview if changes were not saved
//       if (this.user) {
//         this.imagePreview =
//           this.user.image ||
//           'https://www.iconpacks.net/icons/2/free-user-icon-3296-thumb.png';
...
//       email: [userData?.email || '', [Validators.required, Validators.email]],
//       isActive: [userData ? userData.isActive : true],
//       // Skills as a comma-separated string in the form
//       skills: [userData?.skills?.map((skill) => skill.name).join(', ') || ''],
//     });
//   }
//   onFileSelected(event: Event): void {
//     const element = event.currentTarget as HTMLInputElement;
...
//     const formValue = this.profileForm.value;
//     const skillNames = formValue.skills
//       ? formValue.skills
//           .split(',')
//           .map((s: string) => s.trim())
//           .filter((s: string) => s)
...
//     const skillObjects: Skill[] = skillNames.map((name: string) => ({ name }));
...
//       email: formValue.email,
//       isActive: formValue.isActive,
//       skills: skillObjects, // Send skill objects instead of just names
//     };

//     // Using `updateUser` which takes id and data
//     this.userService.updateUser(this.user._id, updatedUserData).subscribe({
//       next: (response) => {
//         // Assuming backend returns the updated user object, possibly nested e.g. response.user
//         const updatedUserFromServer = response.user || response; // Adjust based on your API
//         this.user = { ...this.user, ...updatedUserFromServer } as User;

//         if (this.selectedFile && this.user) {
//           this.uploadImage(this.user._id, this.selectedFile);
//         } else {
...
//   private uploadImage(userId: string, file: File): void {
//     this.userService.updateUserImage({ _id: userId, image: file }).subscribe({
//       next: (response) => {
//         // Adjust based on your actual API response for image URL
//         const newImageUrl =
//           response.imageUrl || (response.user && response.user.image);
//         if (this.user && newImageUrl) {
//           this.user.image = newImageUrl;
//           this.imagePreview = newImageUrl;
//         } else if (this.user) {
//           // If no specific URL, maybe backend updated it and we just need to reflect potential changes
//           // For now, we assume the preview is already set or we might re-fetch user if image URL isn't directly returned
//         }
//         this.successMessage = 'Profile and image updated successfully!';
//         this.isLoading = false;
//         this.isEditing = false;
...
//       error: (err: HttpErrorResponse) => {
//         console.error('Error uploading image:', err);
//         this.errorMessage = `Profile data updated, but image upload failed. ${
//           err.error?.message || err.message
//         }`;
//         this.isLoading = false;
//       },
...
//     ) {
//       const skillNames = this.user.skills
//         // Filter out any malformed skill objects (e.g., null or not an object with a name)
//         .filter(
//           (skill: any): skill is Skill =>
//             skill && typeof skill.name === 'string'
//         )
...
//         .map((skill: Skill) => skill.name)
//         .join(', ');
//       return skillNames.length > 0 ? skillNames : 'N/A'; // If all skills were filtered out, return 'N/A'
//     }
//     return 'N/A'; // Default if no skills or user.skills is not an array
//   }
...
//   // Helper for form control validation messages
//   isInvalid(controlName: string): boolean {
//     const control = this.profileForm.get(controlName);
//     return !!(control && control.invalid && (control.dirty || control.touched));
//   }
